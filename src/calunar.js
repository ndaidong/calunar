...
((name, factory) => {
  ...
  /* Discard the fractional part of a number, e.g., toInt(3.2) = 3 */
  var toInt = (d) => {
    return Math.floor(d);
  };
   ...
   */
  var jdFromDate = (dd, mm, yy) => {
    let a, y, m, jd;
    a = toInt((14 - mm) / 12);
    y = yy + 4800 - a;
    m = mm + 12 * a - 3;
    jd = dd + toInt((153 * m + 2) / 5) + 365 * y + toInt(y / 4) - toInt(y / 100) + toInt(y / 400) - 32045;
    if (jd < 2299161) {
  ...
  var toSunLongitude = (jdn) => {
    ...
    L *= dr;
    L -= PI * 2 * toInt(L / (PI * 2)); // Normalize to (0, 2*PI)
    return L;
  };
   ...
   */
  var getSunLongitude = (dayNumber, timeZone) => {
    return toInt(toSunLongitude(dayNumber - 0.5 - timeZone / 24) / PI * 6);
  };
  ...
  var getNewMoonDay = (k, timeZone) => {
    return toInt(toNewMoon(k) + 0.5 + timeZone / 24);
  };
  ...
  var convertSolar2Lunar = (dd, mm, yy, timeZone) => {
    ...
    let diff = toInt((monthStart - a11) / 29);
    lunarLeap = 0;
    lunarMonth = diff + 11;
    if (b11 - a11 > 365) {
  ...
  var convertLunar2Solar = (lunarDay, lunarMonth, lunarYear, lunarLeap, timeZone) => { // eslint-disable-line max-params
